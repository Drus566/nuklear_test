...
static void draw_gui(void) {
  if (nk_begin(ctx, "Modbus Signals Configuration", nk_rect(50, 50, 450, 300),
  NK_WINDOW_BORDER|NK_WINDOW_MOVABLE|NK_WINDOW_CLOSABLE|NK_WINDOW_TITLE|NK_WINDOW_SCALABLE))
{
  ...
  nk_label(ctx, "Имя (Name)", NK_TEXT_LEFT);
  
  // Вторая колонка заголовка: "Значение" (ширина 120px)
  nk_layout_row_push(ctx, 120);
  nk_label(ctx, "Значение (Value)", NK_TEXT_LEFT);
  ...
  for (int i = 0; i < NUM_SIGNALS; ++i) {
      ...
      nk_layout_row_push(ctx, 120);
      // nk_property_int(контекст, "Лейбл:", мин, &значение, макс, шаг, скорость)
